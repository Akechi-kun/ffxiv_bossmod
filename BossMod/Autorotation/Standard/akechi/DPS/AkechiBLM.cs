            ...
            if (!strategy.HoldAbilities())
            {
                ...
                if (!strategy.HoldCDs())
                {
                    if (!strategy.HoldBuffs())
                    {
                        ...
                        if (mfCondition)
                            QueueOGCD(AID.Manafont, Player, Unlocked(AID.Paradox) || HasInstants(strategy, primaryTarget?.Actor) ? mfPrio : mfPrio + 2004); //4254
                        //LeyLines
                        var (llCondition, llPrio) = ShouldUseLeyLines(llStrat, primaryTarget?.Actor);
                        if (llCondition)
                            QueueOGCD(AID.LeyLines, Player, Unlocked(AID.Paradox) || HasInstants(strategy, primaryTarget?.Actor) ? llPrio : llPrio + 2001); //4251
                        //Amplifier
                    ...
                    if (ShouldUseSwiftcast(strategy.Option(Track.Swiftcast).As<UpgradeStrategy>(), primaryTarget?.Actor))
                        QueueOGCD(AID.Swiftcast, Player, Unlocked(AID.Paradox) || HasInstants(strategy, primaryTarget?.Actor) ? GCDPriority.ModeratelyLow : GCDPriority.ModeratelyLow + 2003); //4253
                    //Triplecast

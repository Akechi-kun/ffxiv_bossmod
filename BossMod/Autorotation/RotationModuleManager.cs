...
public sealed class RotationModuleManager : IDisposable
{
    ...
    public Actor? ResolveTargetOverride(StrategyTarget strategy, int param) => strategy switch
    {
        ...
        StrategyTarget.RangedDPSWithLowestHP => WorldState.Party.WithoutSlot().Where(a => a.Role == Role.Ranged).MinBy(a => a.HPMP.CurHP),
        StrategyTarget.MeleeDPSWithLowestHP => WorldState.Party.WithoutSlot().Where(a => a.Role == Role.Melee).MinBy(a => a.HPMP.CurHP),
        ...
        StrategyTarget.EnemyWithLowestPriority => Player != null ? Hints.PriorityTargets.MaxBy(e => (e.Actor.Position - Player.Position).LengthSq())?.Actor : null,
